...
class DownloadConfig(BaseModel):
    ...
    file_url: HttpUrl = Field(..., description="URL to download the data from")

    artifact_name: str = Field(
        ..., min_length=1, max_length=100, description="Name of the artifact in W&B"
    )

    artifact_type: str = Field(
        ..., min_length=1, description="Type of artifact (raw_data, processed_data, etc.)"
    )

    artifact_description: str = Field(default="", description="Description of the artifact")

    gcs_output_path: str = Field(
        default="data/01-raw/housing.csv", description="Path in GCS (without gs://bucket/)"
    )

    bucket_name: str = Field(
        ..., min_length=3, max_length=63, description="Name of the GCS bucket (without gs://)"
    )
...
class FileStats(BaseModel):
    ...
    missing_values: Optional[Dict[str, int]] = Field(None, description="Missing values per column")
...
class DownloadResult(BaseModel):
    ...
    def gcs_path(self) -> str:
        ...
        return self.gcs_uri.replace("gs://", "").split("/", 1)[1]
